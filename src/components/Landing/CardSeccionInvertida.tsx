...
const CardSeccionInvertida = ({card}:CardSeccionInvertidaProps) => {
  ...
  return (
    <article className={`${color} p-5 space-y-5 rounded-sm shadow-lg`}>
      ...
      <p className='mx-10 text-base sm:text-sm lg:text-lg text-white font-semibold'>
        {card.description}
      </p>
      <br />
      <Link to={card.url} className='mx-10 w-full bg-azul-claro text-white font-semibold rounded-sm p-2'>Ver plan</Link>
    </article>
  )
}
